...
datasets = ['synthetic', 'BATTERY','SMD', 'SWaT', 'SMAP', 'MSL', 'WADI', 'MSDS', 'UCR', 'MBA', 'NAB']
...
def load_data(dataset):
	...
	if dataset == 'synthetic':
		#将原数据集已10000点为界面，划分为训练集和测试集，对于标签来说，在原始的基础上减10000，构造和测试集
		# 一样大小的矩阵，同时上下+30，进行每一个点的标注。对于测试集在label基础上添加正态分布
		...
		split = 10000
		train = normalize(dat.values[:, :split].reshape(split, -1))  #reshape相当于转置
		...
		test += labels * np.random.normal(0.75, 0.1, test.shape)  #正态分布
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
	elif dataset == 'BATTERY':
		train_file = os.path.join(data_folder, dataset, 'battery_train.csv')
		test_file = os.path.join(data_folder, dataset, 'battery_test.csv')
		test_labels_file = os.path.join(data_folder, dataset, 'battery_test_label.csv')

		train_data = pd.read_csv(train_file)
		train_data = train_data.values[:, 1:]
		train_data = np.nan_to_num(train_data)
		sc_x=MinMaxScaler().fit(train_data)
		train = sc_x.transform(train_data)

		test_data = pd.read_csv(test_file)
		test_data = test_data.values[:, 1:]
		test_data = np.nan_to_num(test_data)
		sc_x=MinMaxScaler().fit(test_data)
		test = sc_x.transform(test_data)
	...
	elif dataset == 'UCR':  #单维时间变量
		dataset_folder = 'data/UCR'
		file_list = os.listdir(dataset_folder) #返回指定路径下的文件和文件夹列表。
		for filename in file_list:
	...
	elif dataset == 'SWaT':    #数据集不太靠谱，使用NSIBF贝叶斯滤波中的
		...
		train = pd.read_csv(os.path.join(dataset_folder, 'WADI_14days.csv'), skiprows=1000, nrows=2e5)   #nrows=2和skiprows=2表示从低三行开始读取前两列 （200000,130）
		test = pd.read_csv(os.path.join(dataset_folder, 'WADI_attackdata.csv'))   #（172801,130）
		train.dropna(how='all', inplace=True); test.dropna(how='all', inplace=True)  #dropna()函数的作用是去除读入的数据中（DataFrame）含有NaN的行。
		train.fillna(0, inplace=True); test.fillna(0, inplace=True)  #pad/ffill：用前一个非缺失值去填充该缺失值
