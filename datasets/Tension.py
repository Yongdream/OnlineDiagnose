...
def get_files(root, op=3, signal_size=512):
    '''
...
def data_load(item_path, label, data, lab, signal_size):
    ...
    '''
    fl = pd.read_csv(item_path,encoding='gbk',usecols=[2])
...
class Tension(object):
    num_classes = 2
    inputchannel = 1

    def __init__(self, data_dir, normlizetype, op=0, signal_size=512):
