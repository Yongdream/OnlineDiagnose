...
def data_load(item_path, label, data, lab, signal_size):
    ...
    '''
    fl = pd.read_csv(item_path,encoding='gbk',usecols=[0])
...
class StorageTankLoading(object):
    num_classes = 2
    inputchannel = 1

    def __init__(self, data_dir='D:\IFD\data\StorageTankLoading', normlizetype='mean-std', op=4, signal_size=128):
        self.data_dir = data_dir
        self.normlizetype = normlizetype
        self.op = int(op)
        self.signal_size=signal_size

    #根据工况读取数据集，不同时 读取源域/目标域数据集
...
#train_dataset=SendLineAux().data_preprare(is_src=True)
