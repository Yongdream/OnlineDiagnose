...
class ADACL(nn.Module):
    ...
    def __init__(self, in_channel=1, num_classes=3, num_source=1):
        ...
        self.clf = nn.ModuleList([nn.Linear(128, num_classes) \
                                              for _ in range(num_source)])      #几个源域就有几个分类器
    ...
    def forward(self, target_data, device, source_data=None, source_label=None, source_idx=None):
        if self.training == True:
            ...
            logits = self.clf[source_idx](feat_src)     #第idx个源域输入第idx个分类器
            logits_tgt = [cl(feat_tgt) for cl in self.clf]      #目标域输入每一个源域分类器
            loss_cls = F.cross_entropy(logits, source_label)            #源域分类损失
            ...
            loss_d = F.cross_entropy(logits_dm, labels_dm)              #领域判别器损失（对抗训练）
